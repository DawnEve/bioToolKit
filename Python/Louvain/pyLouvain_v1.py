...
# https://blog.csdn.net/weixin_33805992/article/details/85992666
# 评价: 和 Seurat 结果差距较大，有些cluster在UMAP上是撕裂的。速度慢。
...
class PyLouvain:
    ...
    '''
    @classmethod
    def from_file(cls, path): # cls : 表示没用被实例化的类本身
        f = open(path, 'r')
        lines = f.readlines()
        f.close()
        nodes = {}
...
if __name__ == '__main__': 
    #dt=PyLouvain.from_file("data/OpenFlights.txt")
    #input_file="data/OpenFlights.txt"
    #input_file="data/t1.txt"
    #input_file="data/t2.txt"
    input_file="backup/snn_df.txt"

    dt=PyLouvain.from_file(input_file)
    rs=dt.apply_method()
    print( len(rs[0]) )
