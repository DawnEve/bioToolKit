#!/usr/bin/perl -w
# Example  Generate random DNA
#  using a random number generator to randomly select bases
use strict;
use warnings;
open TRANS ,">$ARGV[3]" or die "$!";
# Declare and initialize the variables
my $size_of_set = $ARGV[0];
my $maximum_length =$ARGV[1];
...
print "with lengths between $minimum_length and $maximum_length,and store in \U$ARGV[3]";
