# Aim: Seurat re-clustering & cell markers using NMF instead of PC to do UMAP
# how to use: $ Rscript this.script.R xx.Seurat.obj [outputRoot [keyword]]
# Env: R 4.2.0
# version: 0.1-20240510 based on do_Seurat.script.R-v0.2
#
# 5k cell may take more than 10 hours using NMF.
# 30k 5days!!
...
if(0){
  ...
  # $ whereis Rscript
  # $ Rscript do_Seurat.single.R /path/Seurat.obj.Rds /output/dir keyword
  # at least 1 parameters, or stop and give help information.
  # para 4-7 must set All Or None!
  #
  # example: 3k 20-60min; 1645cells 60min; 
  # $ /usr/local/bin/Rscript /home/wangjl2/data/others/zhaoym/script/do_Seurat_NMF.script.R \
...
#6B. NMF fig3----
message(now(), "6B. NMF fig3")

res_filename=paste0(outputRoot, keyword,"_6B_NMF_res.list.Rds"); 
message("res_filename=", res_filename)

library(NMF)
if(! file.exists( res_filename ) ){
  message(now(), "Do nmf, as no res file found: ", res_filename)
  
  # filter data
  vm <- sce@assays$RNA@scale.data
  # sub neg as 0
  vm[vm < 0] <- 0
  dim(vm)
  # rm rows with same number
  vm <- vm[apply(vm, 1, var) > 0, ]
  dim(vm)
  
  # time consuming step: 30k cells use 5days!!
  res <- nmf(vm, rank=15, 
             nrun=1,
             method = "snmf/r", seed = 'nndsvd')
  
  # save
  saveRDS(res, file=res_filename, compress = F)
  message(now(), "nmf() end.")
}else{
  message(now(), "Loading nmf, as res file exists: ", res_filename)
  res = readRDS( file= res_filename )
}
res
...
#
colnames(sce@reductions$nmf@cell.embeddings) = paste0("NMF_", 1:ncol(sce@reductions$nmf@cell.embeddings))
sce@reductions$nmf@key="NMF_"

#
pdf(paste0(outputRoot, keyword, "_03-NMF_Elbow.pdf"), width=5, height=4)
DimPlot(sce, reduction = "nmf")
DimPlot(sce, reduction = "nmf", dims = c(1,3))
DimPlot(sce, reduction = "nmf", dims = c(2,3))
...
#7A. UMAP with PCA(ignore) ----
message(now(), "7. UMAP with NMF")
...
if(pc_num> dim(sce[['nmf']])[2]){
  pc_num = dim(sce[['nmf']])[2]
  warning("pc_num is greater then NMF dimension, so it's coerced to ", pc_num)
}

sce <- RunUMAP(sce, reduction = 'nmf', dims = 1:pc_num)
#




#8. cell cluster with NMF fig4----
message(now(), "8. cell cluster fig4")
sce <- FindNeighbors(object = sce, reduction = 'nmf', dims=1:pc_num) #NMF
...
DimPlot(sce, reduction = "umap", label=T)+
  ggtitle( sprintf("HVG:%s, NMF:%s, res:%s", n_HVG, pc_num, resolution) )
...
message(now(), "End of do Seurat NMF script, keyword:", keyword, 
